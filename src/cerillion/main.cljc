(ns cerillion.main
  "Kotodama WASM entrypoint for the Cerillion clean-room actor (L4) — Clojure port.
  ...
  facts (`cerillion.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cerillion")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"cerillion.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cerillion-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cerillion-compat/ready)
